...
//        }
    ...
    public JwtAuhenticationResponse singIn(SigninRequest request) {
        Account checkEmail = accountRepository.findByEmail(request.getEmail()).orElse(null);
        if (checkEmail == null) {
            throw new RestApiException("Tài khoản hoặc mật khẩu không chính xác!");
        }

        if (!passwordEncoder.matches(request.getPassword(), checkEmail.getPassword()) && checkEmail != null) {
            throw new RestApiException("Tài khoản hoặc mật khẩu không chính xác!");
        }
        var jwt = jwtSerrvice.genetateToken(checkEmail);
        var refreshToken = jwtSerrvice.genetateRefreshToken(new HashMap<>(), checkEmail);
